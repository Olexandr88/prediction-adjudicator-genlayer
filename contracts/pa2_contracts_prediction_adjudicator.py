...
# How long a claim can sit without reaching a terminal state before its
# stakers are allowed to walk away with their own money instead of it
# being stuck forever. This is deliberately generous — it's a safety
# valve for claims nobody ever gets around to resolving, not a normal
# path.
RECLAIM_AFTER = datetime.timedelta(days=7)
...
class Claim:
    ...
    submitted_at: datetime.datetime  # for the reclaim-after-timeout safety valve
...
class PredictionAdjudicator(gl.Contract):
    ...
    Two safety valves protect staked GEN beyond the basic lifecycle:
    if nobody ever resolves a claim, stakers can reclaim their own money
    after a fixed timeout rather than have it stuck forever; and if a
    claim finalizes on a side nobody staked on, stakers are refunded
    instead of the contract dividing by zero.
    """
...
"""
    ...
    def claim_winnings(self, claim_id: u32) -> u256:
        ...
        - status == "finalized" and verdict in ("true", "false"): winners
          ...
          If the winning side has zero total stake (nobody backed the
          side that ended up correct), there is no valid ratio to split
          by — everyone is refunded their own stake instead, rather than
          the contract dividing by zero.
        ...
        if claim.status == "finalized":
            ...
            if claim.verdict == "true":
                if claim.pool_true == u256(0):
                    # Nobody staked on the side that won — no ratio to
                    # split by. Refund everyone's own stake instead of
                    # dividing by zero.
    ...
    @gl.public.write
    def reclaim_stake(self, claim_id: u32) -> u256:
        """
        Recovery path for a claim nobody ever finished resolving. If a
        claim is still "pending" or "resolved" (never reached a terminal
        state) after RECLAIM_AFTER has elapsed since it was submitted,
        any staker can reclaim their own stake in full — the pooled GEN
        is never permanently locked just because no one called resolve()
        enough times to reach a verdict.

        Reverts if the claim is already finalized or disputed (use
        claim_winnings() instead — those states already have their own
        payout/refund path) or if the deadline hasn't passed yet.
        """
        if claim_id not in self.claims:
            raise gl.vm.UserError("Unknown claim id")
        claim = self.claims[claim_id]
        ...
        if claim.status in ("finalized", "disputed"):
            raise gl.vm.UserError(
                "Claim already reached a terminal state — use "
                "claim_winnings() instead"
            )

        if datetime.datetime.now() - claim.submitted_at < RECLAIM_AFTER:
            raise gl.vm.UserError(
                "This claim's reclaim deadline hasn't passed yet — give "
                "resolve() more time, or wait out the timeout"
            )
